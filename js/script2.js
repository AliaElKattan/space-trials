...
function createPlanet(){
	...
	planet.mesh.position.y = planetY;
...
function createTrash(){
	...
	for(let i=0;i<=ntrash;i++) {
		...
		trash[i].mesh.position.z = 150 + Math.random() * -800;
...
const R = camStartY + planetY;

const camStep = (Math.PI / 360);
...
function updateCamera(event) {
	...
	// camera.position.x -= (R * Math.cos(camStep));
	// camera.position.y += (R * Math.sin(camStep));
